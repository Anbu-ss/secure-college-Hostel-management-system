...
import { Check, X as RejectIcon, User, X } from 'lucide-react';
...
const StaffPending = () => {
  ...
  const fetchRequests = async () => {
    try {
      const res = await axios.get('/api/requests/staff/pending');
  ...
  const handleAction = async (id, status, reason = null) => {
    setProcessing(id);
    try {
      await axios.put(`/api/requests/staff/approve/${id}`, { status, reason });
  ...
  return (
    <Layout>
      <div className="max-w-6xl mx-auto">
        <h1 className="text-2xl font-bold text-gray-900 mb-2">Pending Approvals — Staff</h1>
        <p className="text-sm text-gray-500 mb-6">Showing <span className="font-semibold text-purple-600">Home Outpass</span> requests awaiting your review.</p>

        <div className="bg-white shadow overflow-hidden sm:rounded-md">
          {loading ? (
            <div className="p-6 text-center text-gray-500">Loading requests...</div>
          ) : requests.length === 0 ? (
            <div className="p-6 text-center text-gray-500">No pending requests to approve.</div>
          ) : (
                  ...
                  <div className="flex items-center justify-between flex-wrap gap-4">
                    <div className="flex items-start flex-1 min-w-0">
                      <div className="flex-shrink-0 mt-1">
                        <User className="h-10 w-10 text-gray-300 rounded-full bg-gray-100 p-2" />
                      </div>
                      <div className="ml-4 flex-1">
                        <div className="flex items-center space-x-2">
                          <h4 className="text-lg font-bold text-gray-900">{req.Name} <span className="text-sm font-normal text-gray-500">({req.RegisterNumber})</span></h4>
                          <span className="px-2 inline-flex text-xs font-semibold rounded-full bg-purple-100 text-purple-700">🏠 Home Outpass</span>
                          {req.StudentDept && (
                            <span className="px-2 inline-flex text-xs font-semibold rounded-full bg-gray-100 text-gray-700">{req.StudentDept}</span>
                          )}
                        </div>
                        <div className="mt-1 flex flex-col sm:flex-row sm:space-x-4 text-sm text-gray-500">
                          <p>Block: {req.Block}, Room: {req.RoomNumber}</p>
                        </div>
                    ...
                    <div className="flex space-x-2">
                       <button
                         onClick={() => handleAction(req.ID, 'StaffApproved')}
                       ...
                       <button
                         ...
                         className="inline-flex items-center px-4 py-2 border border-transparent text-sm font-medium rounded-md text-white bg-red-600 hover:bg-red-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-red-500 shadow-sm disabled:opacity-50"
                       >
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-md p-6 overflow-hidden">
            ...
            <p className="text-sm text-gray-500 mb-4">
              Provide a reason for rejecting this Home Outpass. This note will be visible to the student.
            </p>
            ...
            <textarea
              ...
              rows={4}
              placeholder="e.g. Incomplete details, attendance issues, or pending academic tasks..."
              value={rejectReason}
